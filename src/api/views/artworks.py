...
class ArtworksViewSet(viewsets.GenericViewSet):
    ...
    def download(self, request, *args, pk=None, **kwargs):
        ...
        metadata_content = (
            ...
            f'{artwork._meta.get_field("title_comment"+lang_label).verbose_name.title()}: {apply_strikethrough(artwork.title_comment_localized)}\n'
            f'{metadata_persons}'
            f'{artwork._meta.get_field("date").verbose_name.title()}: {artwork.date}\n'
            f'{artwork._meta.get_field("material"+lang_label).verbose_name.title()}: {artwork.material_description_localized}\n'
            f'{artwork._meta.get_field("dimensions_display").verbose_name.title()}: {artwork.dimensions_display}\n'
            f'{artwork._meta.get_field("comments"+lang_label).verbose_name.title()}: {apply_strikethrough(artwork.comments_localized)}\n'
